...
/**
 * SparkCompute that uses a trained model to tag input records using Linear regression.
 */
@Plugin(type = SparkCompute.PLUGIN_TYPE)
@Name(LinearRegressionPredictor.PLUGIN_NAME)
@Description("Uses a Linear Regression model to make predictions.")
public class LinearRegressionPredictor extends SparkCompute<StructuredRecord, StructuredRecord> {
  public static final String PLUGIN_NAME = "LinearRegressionPredictor";
  private LinearRegressionPredictorConfig config;
  private Schema outputSchema;
  private LinearRegressionModel loadedModel;
  ...
  public void initialize(SparkExecutionPluginContext context) throws Exception {
    ...
    loadedModel = LinearRegressionModel.load(sparkContext, modelLocation.toURI().getPath());
  }
  ...
  public JavaRDD<StructuredRecord> transform(SparkExecutionPluginContext context,
    ...
    final Schema schema = input.first().getSchema();
    ...
    JavaRDD<StructuredRecord> output = input.map(new Function<StructuredRecord, StructuredRecord>() {
      ...
      public StructuredRecord call(StructuredRecord record) throws Exception {
        ...
        return SparkUtils.cloneRecord(record, schema, config.predictionField)
  ...
  /**
   * Configuration for the LinearRegressionPredictor.
   */
  public static class LinearRegressionPredictorConfig extends PluginConfig {
    ...
    @Nullable
    @Description("A comma-separated sequence of fields for Linear Regression. If empty, all fields will be used for " +
      "prediction. Features to be used, must be from one of the following types: int, long, float or double. Both" +
      " featuresToInclude and featuresToExclude fields cannot be specified.")
    private final String featuresToInclude;

    @Nullable
    @Description("A comma-separated sequence of fields to be excluded when calculating for prediction. If empty, all " +
      "fields will be used for prediction. Both featuresToInclude and featuresToExclude fields cannot be " +
      "specified.")
    ...
    public LinearRegressionPredictorConfig(String fileSetName, String path, @Nullable String featuresToInclude,
