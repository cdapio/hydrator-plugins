...
| Destination Directory | Yes       | n/a     | The valid full HDFS destination path in the same cluster where
...
| Overwrite | No       | `true`     | Boolean that specifies if any matching files already present in the
destination should be overwritten or not |
