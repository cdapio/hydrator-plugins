...
public class FileActions {
  ...
  public static void overrideOutputSchema(String jsonOverrideFieldList) {
    Map<String, String> outputSchema =
      JsonUtils.convertKeyValueJsonArrayToMap(PluginPropertyUtils.pluginProp(jsonOverrideFieldList));
    for (Map.Entry<String, String> entry : outputSchema.entrySet()) {
      ElementHelper.clickOnElement(FileLocators.outputSchemaDataTypeDropdown(entry.getKey()));
      SeleniumDriver.getDriver()
        .findElement(FileLocators.outputSchemaDataTypeOption(entry.getKey(), entry.getValue())).click();
    }
  }
}
