...
/**
 * {@link LookupProvider} implementation for {@link ValueMapper}.
 */
public class ValueMapperLookupProvider {

  private final LookupProvider delegate;
  private final LookupConfig config;

  public ValueMapperLookupProvider(LookupProvider delegate, LookupConfig config) {
    this.delegate = delegate;
    this.config = config;
  }

  @SuppressWarnings("unchecked")
  public ValueMapperLookUp provide(String table) {
    if (config == null) {
    ...
    return new ValueMapperLookUp(delegate.provide(table, arguments.getProperties()),
            tableConfig);
  }
